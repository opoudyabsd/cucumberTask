...
When(
  /^User enters a board "(.*)" into "Board title" form$/,
  async (titleBoard) => {
...
Then(
  /^User should be redirected to the '"(.*)" | Trello' page$/,
  async (titleBoard) => {
    await expect(browser).toHaveTitle(`${titleBoard} | Trello`);
  }
);
Then(
  /^New board with specified "(.*)" should be created$/,
  async (titleBoard) => {
...
When(/^User enters a valid list "(.*)"$/, async (name) => {
...
Then(
  /^New list with specified list "(.*)" should be created$/,
  async (name) => {
    await expect(boardsPage.createList.checkListName(name)).toBeDisplayed();
  }
);
Then(
  /^Corresponding list with specified list "(.*)" should be displayed on the board window$/,
  async (name) => {
...
When(/^User should be on the existing board page$/, async () => {
  await browser.url(boardsPage.existingBoardUrl);
});
...
When(
  /^User clicks on the "Add a card" button under existing list$/,
  async () => {
...
When(/^User enter a valid "(.*)" in form$/, async (name) => {
...
Then(
  /^New card with specified list "(.*)" should be created and displayed$/,
  async (name) => {
